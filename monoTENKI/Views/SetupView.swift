...
struct SetupView: View {
  ...
  var body: some View {
    TabView(selection: $selection) {
      ...
      GuideView(
        ...
        action: {
          Button(DesignSystem.AppText.SetupText.greetingsButton) { selection += 1 }
      ...
      GuideView(
        ...
        action: {
          Button(DesignSystem.AppText.SetupText.permissionButtonGrand) { requestLocationAuthorization() }
          .buttonStyle(.monoBordered)
          .onChange(of: locationManager.isAuthorized) { setWeatherToCurrentLocation() }

          Button(DesignSystem.AppText.SetupText.permissionButtonDeny) { selection += 1 }
          .buttonStyle(.monoBordered)
        })
      .tag(1)
  ...
  private func requestLocationAuthorization() {
    ...
    selection = locationManager.isAuthorized ? selection + 2 : selection
  }
